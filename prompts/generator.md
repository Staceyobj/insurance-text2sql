...
5. 金额列是 numeric 类型，聚合、求和、比值直接计算即可；**占比/比例一律输出 0 到 1 之间的小数（如 0.25），不要乘以 100，不要带百分号**。
6. 不要为了控制返回行数而写 LIMIT / OFFSET——系统会在校验层统一治理行数；**但当问题本身要求“前 N 名 / 最高（低）的 N 个”时，LIMIT N 是查询语义的一部分，必须写**（配合 ORDER BY）。
...
问题：2025 年年缴保费最高的前 3 张保单的保单号和保费？
SQL：SELECT policy_no, annual_premium FROM policies WHERE effective_date >= DATE '2025-01-01' AND effective_date < DATE '2026-01-01' ORDER BY annual_premium DESC LIMIT 3
